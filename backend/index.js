...
app.get('/get-event-details/:eventId', (req, res) => {
  const eventId = req.params.eventId;

  // Log caseId to console for debugging
  console.log('Event ID:', eventId);

  // Fetch specific columns from the database for a particular event
  const selectQuery = "SELECT * FROM `event` WHERE event_id = ?";

  connection.query(selectQuery, [eventId], (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ success: false, message: 'Internal Server Error' });
    } else if (result.length === 0) {
      // If no event is found with the given caseId, return a 404
      return res.status(404).json({ success: false, message: 'Event not found' });
    } else {
      // Log the fetched data in the console
      console.log('Fetched event details:', result[0]);

      // Send the retrieved case details as a JSON response
      return res.status(200).json({ success: true, eventData: result[0] });
    }
  });
});
